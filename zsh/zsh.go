...
%[2]s() {
    ...
    for line in "${lines[@]}"; do
        case "$line" in
            $'\001files') [[ $PREFIX = -*=* ]] && compset -P '[^=]#='; _files; return ;;
            $'\001dirs')  [[ $PREFIX = -*=* ]] && compset -P '[^=]#='; _files -/; return ;;
            '') ;;
            *) pairs+=("$line") ;;
        esac
    done
...
func answer(w io.Writer, src engine.Source, cfg config, words []string) {
	...
	// zsh's compadd matches candidates against the WHOLE current word
	// ($PREFIX): a value completed through --name= must be rebuilt to
	// the full token or _describe never matches it. First = — the
	// engine's own semantic cut.
	eq := -1
	...
	for _, cand := range engine.Complete(src, q) {
		if cand.Kind == engine.KindFiles {
			...
			full := cand.Value
			if cand.Kind == engine.KindValue && eq >= 0 {
				full = cfg.Current[:eq+1] + cand.Value
			}
			line := strings.ReplaceAll(full, ":", `\:`)
